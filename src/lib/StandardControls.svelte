<script lang="ts">
	...
	let primary_commands = [
		{ click: () => model.home(), name: 'Home', classes: ['bg-purple-400 border-purple-500'], icon: 'home' },
		{ click: () => model.unlock(), name: 'Unlock', classes: ['bg-cyan-400 border-cyan-500'], icon: 'unlock' },
		{ click: () => model.sleep(), name: 'Sleep', classes: ['bg-blue-400 border-blue-500'], icon: 'bed' },
		{ click: () => model.reset(), name: 'Reset', classes: ['bg-orange-400 border-orange-500'], icon: 'repeat' },
		{
			click: () => model.feedhold(),
			name: 'Feedhold',
			classes: ['bg-rose-400 border-rose-800 text-rose-900'],
			icon: 'circle-stop'
		},
